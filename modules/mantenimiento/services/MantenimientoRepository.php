...
class MantenimientoRepository
{
        ...
        try {
            $stmt = $this->conn->prepare(
                "UPDATE mantenimientos SET fecha_inicio = :inicio, fecha_fin = :fin, duracion_horas = :duracion,
                    observaciones_finales = :observaciones, costo_mano_obra = :mano_obra, costo_repuestos = :repuestos,
                    costo_total = :total, estado = :estado
                 WHERE id = :id"
            );
            $stmt->execute([
                ...
                ':mano_obra' => $data['costo_mano_obra'],
                ':repuestos' => $data['costo_repuestos'],
            ...
            if (!empty($repuestos)) {
                $stmtInsert = $this->conn->prepare(
                    "INSERT INTO mantenimiento_repuestos (mantenimiento_id, nombre_repuesto, cantidad, costo_unitario)
                     VALUES (:mantenimiento_id, :nombre, :cantidad, :costo)"
                );
    ...
    {
        $stmt = $this->conn->prepare("SELECT * FROM mantenimiento_repuestos WHERE mantenimiento_id = :id ORDER BY nombre_repuesto");
        $stmt->execute([':id' => $mantenimientoId]);
