...
$totalRepuestos = array_sum(array_map(fn($item) => (int) ($item['total_repuestos'] ?? 0), $mantenimientos));
$totalUnidades = array_sum(array_map(fn($item) => (float) ($item['total_unidades'] ?? 0), $mantenimientos));
...
if ($export === 'excel') {
    ...
    fputcsv($output, ['Folio', 'Equipo', 'Tipo', 'Estado', 'Técnico', 'Fecha recepción', 'Repuestos registrados', 'Unidades totales', 'Conocimiento']);
    foreach ($mantenimientos as $registro) {
        fputcsv($output, [
            ...
            (int) ($registro['total_repuestos'] ?? 0),
            number_format((float) ($registro['total_unidades'] ?? 0), 2),
            !empty($registro['conocimiento_numero']) ? ($registro['conocimiento_numero'] . ' (' . ($registro['conocimiento_estado'] ?? '') . ')') : 'Sin generar',
        ]);
    }
    fclose($output);
    exit();
...
if ($export === 'pdf') {
    ...
    $html .= '<p>Generado el ' . date('d/m/Y H:i') . ' — Total registros: ' . $totalRegistros . ' — Repuestos registrados: ' . $totalRepuestos . ' — Unidades totales: ' . number_format($totalUnidades, 2) . '</p>';
    $html .= '<table border="1" cellpadding="4" cellspacing="0">';
    $html .= '<thead><tr style="background-color:#f0f0f0; font-weight:bold;"><th>Folio</th><th>Equipo</th><th>Tipo</th><th>Estado</th><th>Técnico</th><th>Recepción</th><th>Repuestos</th><th>Unidades</th><th>Conocimiento</th></tr></thead><tbody>';
    foreach ($mantenimientos as $registro) {
        $html .= '<tr>';
        ...
        $html .= '<td>' . (int) ($registro['total_repuestos'] ?? 0) . '</td>';
        $html .= '<td>' . number_format((float) ($registro['total_unidades'] ?? 0), 2) . '</td>';
        $conocimientoTexto = !empty($registro['conocimiento_numero']) ? ($registro['conocimiento_numero'] . ' (' . ($registro['conocimiento_estado'] ?? '') . ')') : 'Sin generar';
        $html .= '<td>' . htmlspecialchars($conocimientoTexto) . '</td>';
        $html .= '</tr>';
    }
    if (empty($mantenimientos)) {
        $html .= '<tr><td colspan="9">No se encontraron registros con los filtros seleccionados.</td></tr>';
    }
    $html .= '</tbody></table>';
...
<div class="card">
    <header class="card-header">
        <div class="card-header-title is-justify-content-space-between is-align-items-center">
            ...
            <span class="tag is-info is-light">Total: <?php echo $totalRegistros; ?> — Repuestos: <?php echo $totalRepuestos; ?> — Unidades: <?php echo number_format($totalUnidades, 2); ?></span>
        </div>
    </header>
                ...
                <table class="table is-fullwidth is-striped is-hoverable">
                    <thead>
                        <tr>
                            ...
                            <th class="has-text-centered">Unidades</th>
                    ...
                    <tbody>
                        <?php foreach ($mantenimientos as $registro): ?>
                            <tr>
                                ...
                                <td class="has-text-centered"><?php echo (int) ($registro['total_repuestos'] ?? 0); ?></td>
                                <td class="has-text-centered"><?php echo number_format((float) ($registro['total_unidades'] ?? 0), 2); ?></td>
                                <td class="has-text-centered">
                                    <?php if (!empty($registro['conocimiento_id'])): ?>
                                        <a class="has-text-weight-semibold" href="<?php echo APP_URL; ?>/modules/conocimientos/pages/editar.php?id=<?php echo (int) $registro['conocimiento_id']; ?>">
                                            #<?php echo htmlspecialchars($registro['conocimiento_numero'] ?? ''); ?>
                                        </a>
                                        <p class="is-size-7 has-text-grey"><?php echo htmlspecialchars($registro['conocimiento_estado'] ?? ''); ?></p>
                                    <?php else: ?>
